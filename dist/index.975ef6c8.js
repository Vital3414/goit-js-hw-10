...
refs.input.addEventListener("input", (0, _lodashDefault.default).debounce(onInputChange, DEBOUNCE_DELAY));
function onInputChange(e) {
    refs.countryList.innerHTML = "";
    refs.countryCard.innerHTML = "";
    if (!e.target.value.trim()) return;
    (0, _fetchCountries.fetchCountries)(e.target.value.trim()).then(showResult).catch(()=>(0, _notiflixDefault.default).Notify.failure("Oops, there is no country with that name"));
}
function showResult(data) {
    if (data.length > 10) {
        (0, _notiflixDefault.default).Notify.info("Too many matches found. Please enter a more specific name.");
        return;
    } else if (data.length > 1) refs.countryList.innerHTML = createCountryListMarkup(data);
    else refs.countryCard.innerHTML = createCountryCardMarkup(...data);
}
...
<div class="country-card">
  ...
  <div class="country-card__info">
    ...
    <p><span class="country-card__info-title">Population:</span> ${country.population}</p>
    <p><span class="country-card__info-title">Languages:</span> ${country.languages.map((lang)=>lang.name).join(", ")}</p>
  </div>
</div>`;
}

},{"./css/styles.css":"1CY4s","lodash":"3qBDj","notiflix":"5z0Oc","./fetchCountries":"fTQqv","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}],"1CY4s":[function() {},{}],"3qBDj":[function(require,module,exports) {
var global = arguments[3];
(function() {
...
function fetchCountries(query) {
    ...
    return fetch(`https://restcountries.com/v2/name/${sQuery}`).then((res)=>res.json());
}
